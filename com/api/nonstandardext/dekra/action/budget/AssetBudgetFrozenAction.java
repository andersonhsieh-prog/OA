 ...
 * 流程提交冻结预算
 */
public class AssetBudgetFrozenAction extends BaseBean implements Action {
	...
	public String doBudgetDt1(String requestId, String mainTablename, String mainId, String wfCreateTime){
		...
		while (rs.next()){
			...
			if (!"".equals(ysbm)){
				String yuguFee = Util.null2String(rs.getString("ygcgbhsjermb"));
				if ("".equals(yuguFee)){
					return "预估采购不含税金额不能为空";
				}

				this.writeLog(Action_Name + "  yuguFee :" + yuguFee);
				BigDecimal currentUseBudget = currentUseBudgetMap.get(ysbm);
				if (currentUseBudget == null){
					currentUseBudget = new BigDecimal(yuguFee);
				} else {
					currentUseBudget = currentUseBudget.add(new BigDecimal(yuguFee));
				}
		...
		if (currentUseBudgetMap.size() > 0){
			...
			this.writeLog(Action_Name + " result :" + result);
